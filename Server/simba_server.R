...
server <- function(input, output, session) {
  ...
  packages.py <- c("networkx","pandas","numpy","typing")
  py_install(packages.py)
  ...
  # Jointure des fichiers des séquences et individus
  ...
  myData <- reactive({
                ...
                if (!(is.null(inFile_seq) | is.null(inFile_ind))) {
                    ...
                    if (ncol(file_seq) > 1 && ncol(file_ind) > 1) {
                        tryCatch({
                            val$file_filtre <- merge(file_seq, file_ind, by = "Id_ind", all.x = TRUE)
                        },
                    
                    # -- Notification des erreurs -- #
                    error = function(e) {
                        message <- "Fichier(s) incorrect(s).\nAssurez-vous d'avoir \"Id_ind\" dans les deux fichiers"
  ...
  # Affichage chord diagram 
   ...
   # Affichage daily pattern
  
   output$daily_pattern <- daily_pat(myInputs, myData, input, session, output, val)
   
   output$network <- daily_net(myInputs, myData, input, session, output, val) 
  ...
  output$mosaic <-  mosaic_plot_affichage(myInputs, myData, input, session, output, val)  
  ...
  output$print_mosaic_clusters <- renderPlot({
    file <- val$clust_mk
    cond <- ""
